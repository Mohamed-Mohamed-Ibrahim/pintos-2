...
void get_arguments(struct intr_frame *f, int *arguments, int number_of_arguments){
    for (int i = 0; i < number_of_arguments; ++i) {
        int* stack_top_ptr = (int *) f->esp + i + 1;
        check_address((const void *) stack_top_ptr);
        arguments[i] = *stack_top_ptr; // store the value at the address
    }
}

void check_address(const void *stack_top_pointer)
{
    if (!is_user_vaddr(stack_top_pointer) || stack_top_pointer < (void *) 0x08048000) {
